...
export class TimesheetApprovedComponent {
    ...
    loadTimesheetApprovedYear(): void {
        this.TimesheetApprovedService.getAllTimesheetApprovedData().subscribe(
            (res: TimesheetApprovedModel[]) => {
                const ds = res.map((item, index) => ({ ...item, id: index + 1 }));
                // this.dataSource.data = ds;

                this.yearNumber = [
                    ...new Set(ds.map((item) => new Date(item.start_date).getFullYear())),
                ];
            }
        );
    }
    ...
    loadAllTimesheetApproved() {
        ...
        this.TimesheetApprovedService.getAllemployeetData().subscribe((employeeData: any) => {
            ...
            if (user) {
                ...
                this.TimesheetApprovedService.getAllTimesheetApprovedDataByYearandWeek(
                    this.latestYearNumber,
                    this.latestWeekNumber
                ).subscribe((timesheetData: any) => {
                    if (isOwner) {
                        // If user is an owner, display all timesheets
                        this.dataSource = new MatTableDataSource<TimesheetApprovedModel>(
                            timesheetData
                        );
                        this.dataSource.paginator = this.paginator;
                        console.log(timesheetData);

                        const ds = timesheetData.map((item: any, index: number) => ({
                            ...item,
                            id: index + 1,
                        }));
                        ...
                        const ds = filteredTimesheets.map((item: any, index: number) => ({
                            ...item,
                            id: index + 1,
                        }));
                        const filteredData = ds.filter((data: any) => {
