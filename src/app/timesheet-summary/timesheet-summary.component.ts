...
export class TimesheetSummaryComponent {
    ...
    loadFilterSummaryYear() {
        this.timesheetSummaryService
            .getAllSummaryData()
            .subscribe((res: TimesheetSummaryModel[]) => {
    ...
    loadTimesheetSummary() {
        this.timesheetSummaryService
            .getFilteringYearAndWeek(this.latestWeekNumber, this.latestYearNumber)
            .subscribe((res: TimesheetSummaryModel[]) => {
                const ds = res.map((item, index) => ({ ...item, id: index + 1 }));
                this.dataSource.data = ds;
            });
    ...
    selectWeekNumber(event: any) {
        if (event == null) {
            ...
            this.timesheetSummaryService
                .getFilteringYearAndWeek(weekFilterValue, filterYear)
                .subscribe((res: TimesheetSummaryModel[]) => {
                    const ds = res.map((item, index) => ({ ...item, id: index + 1 }));
                    this.dataSource.data = ds;

                    this.dataSource.filterPredicate = (
                        data: TimesheetSummaryModel,
                        filter: string
                    ) => {
    ...
    selectYearNumber(event: any) {
        if (event == null) {
            ...
            this.timesheetSummaryService
                .getFilteringYearAndWeek(filterWeek, yearFilterValue)
                .subscribe((res: TimesheetSummaryModel[]) => {
                    const ds = res.map((item, index) => ({ ...item, id: index + 1 }));
                    this.dataSource.data = ds;
